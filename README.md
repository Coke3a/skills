...
Within each plugin, skill names use a language prefix only when the guidance is truly
language-specific; cross-language workflows use capability names such as `tdd-`, `performance-`,
and `ci-cd-`.
...
| `performance-optimization` | Investigating a measured latency, throughput, CPU, memory, I/O, query, or bundle-size problem |
...
| `tdd-feature-workflow` | Implementing observable behavior one acceptance criterion at a time with red/green/refactor |
