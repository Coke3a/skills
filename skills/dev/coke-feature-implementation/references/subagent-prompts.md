...
- **Implementation lens** (user-selected for this project): <list — e.g.,
  `/coke-rust-clean-architecture`, `/coke-rust-performance-optimization`,
  `/react-best-practices`, `/coke-nextjs-app-architecture`, etc.>
...
- **Review lens** (user-selected): <list — e.g., `/coke-rust-code-review`,
...
- Implementer dispatch for a Rust file → only `/coke-rust-*` entries from `implementation_skills`
- Implementer dispatch for a `.tsx` file → only `/coke-nextjs-*`, `/react-*`, `/coke-tdd-*` (if frontend TDD) entries
