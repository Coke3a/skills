...
`coke-go-tdd-feature-workflow` opens with a **hard entry gate** that stops dead unless the
...
`coke-go-tdd-feature-workflow` needs its four facts **quotable from a source**, which stated
...
| A spec already exists — check it before trusting it | `coke-spec-review` |
| Unfamiliar workspace | `coke-workspace-orientation`, first |
...
`coke-go-tdd-feature-workflow` demands before it will write a line: **endpoints ·
...
Same three lanes, different skills. **`coke-feature-implementation` drives the whole feature
once** — not once per lane, and not once per batch.

| Lane | Stage | Skill |
|---|---|---|
| **backend** | test first — the loop that decides the signatures | `coke-go-tdd-feature-workflow` |
| **backend** | structure, naming, error taxonomy, the sqlc/pgx pattern | `coke-go-clean-architecture` |
...
| UI edge states a screenshot never reaches | `coke-design-scrutinize` |
...
| Multi-session or cross-repo work | `coke-project-progress` |
...
| | |
|---|---|
| ⚠ **`coke-go-clean-architecture` mandates goose — in six places** | `db/migrations/` with `-- +goose Up` sections, wired into its `templates/sqlc.yaml`. **This baseline is golang-migrate, and `migration/` is at the repository root.** Following the skill unedited produces a second migrations directory that sqlc reads and nobody runs. The two-line `templates/sqlc.yaml` here is the fix |
...
| ⚠ **`coke-go-tdd-feature-workflow` puts the repository layer out of scope, deliberately** | its handler tests wire a real usecase to a **fake** repository, so grants, RLS, constraints and pgx mapping are proven by **nothing in the skill set at all.** `make test-db` is where that closes — and is the reason that gate exists rather than being a nice-to-have |
| ⚠ **`coke-go-clean-architecture` says to use a transaction only for multi-write atomicity** | *"Use a transaction only when one usecase requires multiple writes to commit atomically."* ⚠ **Under RLS that is false, and its canonical `gen.New(pool)` constructor is the shape that fails silently** — see `conventions.md` |
...
| | |
|---|---|
| ⚠ **`coke-go-performance-optimization`** | measurement-first by its own first rule. Invoke it when a **stated budget has been measured and missed** — never speculatively, and never while building a feature |
