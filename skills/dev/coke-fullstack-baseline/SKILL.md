...
|---|---|
| Where directories go and what each may never contain | Go feature architecture → `coke-go-clean-architecture` |
| Which versions, and the peer constraints that decide them | The Go test loop → `coke-go-tdd-feature-workflow` |
...
| The gate set and the CI shape up to the artifact | Shipping the artifact → `coke-docker-vm-deploy` (or whatever the target is) |
...
- The stack is Next.js → `coke-nextjs-app-architecture`. This skill is Vite SPA + Go API.
...
merely inconvenient without them — `coke-go-tdd-feature-workflow` opens with a hard entry
...
**Phase B · implement** — `coke-feature-implementation` drives the whole feature once.

| Lane | Skill |
|---|---|
| backend, test first | `coke-go-tdd-feature-workflow` |
| backend, structure | `coke-go-clean-architecture` |
...
1. **Orient.** Is this a fresh project, an existing one, or a debug? For an unfamiliar
   workspace, `coke-workspace-orientation` runs first.
...
- `coke-workspace-orientation` — run first in an unfamiliar or multi-repository workspace.
- `coke-feature-implementation` — drives a feature from spec to code once Phase B starts.
- `coke-spec-review` — check a design before trusting it.
- `coke-docker-vm-deploy` — the deploy half this skill deliberately does not carry.
