...
- Use `aria-live="polite"` for async validation, toast, or save status updates when screen reader
  users need the update.
...
- Icons inside icon-only buttons need an accessible button label and usually `aria-hidden` on the
  icon.
