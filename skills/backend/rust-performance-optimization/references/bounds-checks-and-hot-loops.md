...
- Add assertions before the loop if that helps bounds-check elimination and remains
  safe.
...
- Use `unsafe` / `get_unchecked` only as a last resort after safe approaches fail and
  benchmarks prove meaningful benefit.
- Unsafe optimization requires explicit safety comments, tests, benchmark evidence, and
  final code review.
