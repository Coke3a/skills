---
name: coke-go-clean-architecture
...
- Driving the red/green/refactor loop → use `coke-go-tdd-feature-workflow`.
- Setting up GitHub Actions or deployment → use a CI/CD skill.
- Producing a final review report → use a code review skill.
- Profiling, benchmarking, or hot-path tuning → use `coke-go-performance-optimization`.
...
- `coke-go-tdd-feature-workflow` — the red/green/refactor loop that fills these layers. It owns test
  level, placement, and order; this skill owns where the code they drive out belongs.
- `coke-go-performance-optimization` — optimizing without breaking layer boundaries.
- `coke-rust-clean-architecture` — the Rust counterpart this architecture mirrors.
