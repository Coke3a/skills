...
- Driving the red/green/refactor loop → use `coke-tdd-feature-workflow`.
- Setting up GitHub Actions or deployment → use `coke-rust-ci-cd`.
- Producing a final review report → use `coke-rust-code-review`.
- Profiling, benchmarking, or hot-path tuning → use `coke-rust-performance-optimization`.
...
- `coke-tdd-feature-workflow` — test design and the red/green/refactor loop.
- `coke-rust-code-review` — final review against these rules.
- `coke-rust-performance-optimization` — optimizing without breaking layer boundaries.
- `coke-rust-ci-cd` — automation that ships the code produced under this skill.
