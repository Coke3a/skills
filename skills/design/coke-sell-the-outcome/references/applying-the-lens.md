...
|---|---|
| Does this make the user see / feel the outcome (vs. the product)? | **coke-sell-the-outcome** (this one) |
| Is the UX / flow / accessibility sound? | `coke-design-scrutinize` |
| How should the whole landing page be designed (structure, visual, CRO)? | `coke-landing-page-design` |
| Is the code correct / well-architected? | `coke-rust-code-review` |
| Is the spec accurate vs. reality? | `coke-spec-review` |

Compose freely: `coke-landing-page-design` drafts the page, then this lens passes over the message; or `coke-design-scrutinize` checks the UX while this checks whether the value lands. Stay in your lane — contribute only the outcome read.
