...
- Implementing a normal feature → use `coke-eng:rust-clean-architecture` + `coke-eng:rust-tdd-feature-workflow`.
...
- `coke-eng:rust-clean-architecture` — layer structure that must be preserved.
- `coke-eng:rust-tdd-feature-workflow` — correctness and regression tests.
- `coke-eng:rust-code-review` — final code review including async/concurrency review.
- `coke-eng:rust-ci-cd` — running benchmarks in CI when appropriate.
