...
| | |
|---|---|
| ⚠ **`coke-eng:go-clean-architecture` mandates goose — in six places** | `db/migrations/` with `-- +goose Up` sections, wired into its `templates/sqlc.yaml`. **This baseline is golang-migrate, and `migration/` is at the repository root.** Following the skill unedited produces a second migrations directory that sqlc reads and nobody runs. The two-line `templates/sqlc.yaml` here is the fix |
