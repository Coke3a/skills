...
| **User said "reconfigure" / "update config" / "reconfigure skills" / "change the config"** | Force re-interview. Overwrite. Do NOT read the old values; ask fresh. |
...
- Skill knowledge informs which patterns the plan recommends (e.g., `/coke-eng:rust-clean-architecture` shapes the task batching by layer; `/react-best-practices` shapes the frontend batches around component boundaries).
