...
- Use client-side fetching only for data that must refresh from client state, browser APIs, realtime
  interactions, or post-hydration UX.
...
- Avoid wrapping the entire page in one generic fallback when smaller boundaries would reveal useful
  content earlier.

## Dedupe And Caching

- Use the project convention for request dedupe, generated clients, `fetch` caching, React cache, or
