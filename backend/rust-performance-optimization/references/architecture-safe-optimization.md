...
- Infra owns IO, DB queries, Diesel rows, pool access, and DB-specific performance
  optimization.
...
- Do not bypass repository traits unless the user explicitly approves an architecture
  change.
- Prefer optimizing inside the correct layer first.
- If optimization requires architecture change, report it as a proposed design change
